...
public class LocalOfferClientService : ApiService, ILocalOfferClientService
{
    ...
    public async Task<PaginatedList<OpenReferralServiceDto>> GetLocalOffers(string status, int? minimum_age, int? maximum_age, int? given_age, string? districtCode, double? latitude, double? longtitude, double? proximity, int pageNumber, int pageSize, string text, string? serviceDeliveries, bool? isPaidFor, string? taxonmyIds, string? languages)
    {
        ...
        if (latitude != null && longtitude != null && proximity != null)
            url = $"api/services?status={status}&latitude={latitude}&longtitude={longtitude}&proximity={proximity}&pageNumber={pageNumber}&pageSize={pageSize}&text={text}";
        else
            url = $"api/services?status={status}&pageNumber={pageNumber}&pageSize={pageSize}&text={text}";
