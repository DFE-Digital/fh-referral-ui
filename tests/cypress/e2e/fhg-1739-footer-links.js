...
When("the user should see and able to click on  footer link {string} links to {string}", (id, url) => {
    cy.get(`[data-testid=${id}]`).should("exist");
    cy.get(`[data-testid=${id}]`).click();
    cy.location('pathname').should('match', new RegExp(url));
});

When("the user should see and able to click on feedback form redirect to form", () => {
